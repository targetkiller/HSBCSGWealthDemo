...
3. **Add other holdings to analyse:** Opens the Add a portfolio sheet. Link the remaining HSBC Unit Trust and FundMax accounts, connect DBS or Standard Chartered, or upload or scan a statement. Accounts are added only after confirmation. Reconnecting a bank reuses its saved account without creating duplicates or replacing edits.
...
BA colleagues can edit accounts, holdings, and portfolio values in one file: **[Sample/Portfolios.csv](Sample/Portfolios.csv)**. Enter an optional `holdingValue` or `portfolioValue` to set a target market value, or leave these blank to calculate values from quantities and prices. Use `availability` to choose `initial`, `linkable`, or `hidden` accounts. See the **[editing guide](Sample/README.md#english-quick-guide)** for examples. Exchange rates, analytics parameters, scenarios, and other settings live separately in **[Sample/Others/](Sample/Others/README.md)**. Relaunch the app after fully closing it to load the configured initial accounts.

- Every fresh launch starts with **3 HSBC accounts**: Singapore Current Account, Singapore (068) Equity Investment Account, and Hong Kong HSBC One Investment Services. Singapore Unit Trust and Hong Kong FundMax remain available for HSBC linking; DBS Singapore and Standard Chartered Hong Kong are added through the other-bank flow. FUTU accounts appear only after a screenshot or statement import is confirmed. The catalog retains a hidden Hong Kong Current Account for configuration purposes.
- Additions, imports, edits, deletions, and display settings are saved locally during the current app session. Moving to the background and returning preserves them. **Fully closing the app and relaunching resets the demo to the three initial accounts**, so every session can repeat the first-time linking and upload experience. Settings → Restore demo data also resets the current session.
...
Core logic can be tested independently on macOS 14 or later, without a simulator. Coverage includes valuation and allocation across currencies, account and holding CRUD, CSV availability validation, cold-launch reset and session persistence, YTD return targets, distinct performance paths, and reference-account equivalence.

`WealthHubUITests` covers top navigation, account selection and comparison, sample statement import and editing, returning to analysis, first-time HSBC and other-bank linking, background preservation, cold-launch reset, and the GIV - L3 flow:
...
Verification on **2026-09-10**: **57 iOS core tests** and **46 standalone macOS core tests** passed. Targeted iPhone UI regressions verified first-time linking from 3 to 7 accounts, background preservation, cold-launch reset and linking again, duplicate prevention, actual FUTU screenshot import, Singapore/Hong Kong reference selection, and linked-account GIV / YTD views. UI tests use isolated local storage with the same cold-launch reset policy as the app.

The latest GIV result bundle is `build/FirstLaunch/GIV.xcresult`; core, first-launch, and import logs are in `build/FirstLaunch/`. Generated results are excluded from the repository. Committed previews are in [`Screenshots/`](Screenshots/), including the Singapore and Hong Kong selectors after linking the available demo accounts.
